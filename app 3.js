...
function renderReussir(){
  ...
  // Utilise la liste officielle privée lorsqu'elle existe, corrige les anciens noms,
  // enlève les doublons et affiche toujours les élèves en ordre alphabétique.
  const sourceStudents=(state.class_students||getPrivateStudents().map(x=>x.name).join('\n'))
    .split(/\n+/).map(x=>x.trim()).filter(Boolean);
  const students=[...new Set(sourceStudents.map(n=>{
    if(n==='Adefarayola') return 'Fara';
    if(n==='Araotanlowooluwa' || n==='Araotanlowooluwa') return 'Araotan';
    return n;
  }))].sort((a,b)=>a.localeCompare(b,'fr',{sensitivity:'base'}));

  // Met à jour la liste de classe afin que les autres outils utilisent aussi les noms corrigés.
  const normalizedClassStudents=students.join('\n');
  if(state.class_students!==normalizedClassStudents){
    state.class_students=normalizedClassStudents;
    if(Array.isArray(state.private_students)){
      state.private_students=state.private_students.map(st=>({
        ...st,
        name:st.name==='Adefarayola'?'Fara':
             (st.name==='Araotanlowooluwa'||st.name==='Araotanlowooluwa')?'Araotan':st.name
      }));
    }
    saveState();
  }
  ...
  const renderPanel=()=>{
    ...
    if(r.tab==='classe'){
      panel.innerHTML=`
        <section class="hh-student-card">
          ...
          <div class="hh-student-grid" id="hhStudentGrid">
            <button class="hh-student all" data-hh-all="true"><b>✓ Tous</b><span>Toute la classe</span></button>
        ...
        <section class="hh-award-card" id="hhAwardCard">
          <div class="hh-section-title">
            <div>
              <strong>Habitude de travail</strong>
              <span id="hhChosenLabel">Aucun élève sélectionné</span>
            </div>
          </div>
      ...
      const studentButtons=[...panel.querySelectorAll('[data-hh-student]')];
      const allButton=panel.querySelector('[data-hh-all]');
      const awardButtons=[...panel.querySelectorAll('[data-hh-index]')];
      const chosenLabel=document.getElementById('hhChosenLabel');

      const refreshSelection=()=>{
        studentButtons.forEach(btn=>{
          btn.classList.toggle('selected',chosen.has(btn.dataset.hhStudent));
        });

        const allSelected=students.length>0 && students.every(n=>chosen.has(n));
        if(allButton) allButton.classList.toggle('selected',allSelected);

        if(chosen.size===0){
          chosenLabel.textContent='Aucun élève sélectionné';
        }else if(allSelected){
          chosenLabel.textContent=`Toute la classe (${students.length})`;
        }else if(chosen.size===1){
          chosenLabel.textContent=`${[...chosen][0]} sélectionné(e)`;
        }else{
          chosenLabel.textContent=`${chosen.size} élèves sélectionnés`;
        }

        awardButtons.forEach(btn=>btn.disabled=chosen.size===0);
      };

      studentButtons.forEach(btn=>btn.onclick=()=>{
        const name=btn.dataset.hhStudent;
      ...
      if(allButton) allButton.onclick=()=>{
        const allSelected=students.length>0 && students.every(n=>chosen.has(n));
        chosen=allSelected?new Set():new Set(students);
        refreshSelection();
      };
      ...
      document.getElementById('hhStudentSearch').oninput=e=>{
        ...
        studentButtons.forEach(btn=>{
          btn.style.display=(btn.dataset.hhStudent||'').toLocaleLowerCase('fr').includes(q)?'':'none';
        });
      };

      awardButtons.forEach(btn=>btn.onclick=()=>{
        ...
        success.textContent=targets.length===students.length
          ? `✓ ${label} ajouté à toute la classe (${targets.length} élèves).`
          : targets.length===1
            ? `✓ ${label} ajouté à ${targets[0]}.`
            : `✓ ${label} ajouté à ${targets.length} élèves.`;
        ...
        setTimeout(()=>{
          const el=document.getElementById('hhSuccess');
      ...
      panel.innerHTML=`
        <section class="hh-list-card">
          <div class="hh-section-title">
            <div><strong>Historique</strong><span>${r.semester===1?'1er':'2e'} semestre · ${semesterObs.length} observation${semesterObs.length===1?'':'s'}</span></div>
          </div>
          <div class="hh-history">
            ${semesterObs.length
              ?semesterObs.map(o=>`<article><strong>${esc(o.student||'Classe')}</strong><span>${esc(o.label||o.key||'')}</span><small>${new Date(o.date).toLocaleDateString('fr-CA')}</small><button data-del-obs="${o.id}">×</button></article>`).join('')
              :'<p class="muted-copy">Aucune observation pour ce semestre.</p>'}
          </div>
        </section>`;

      document.querySelectorAll('[data-del-obs]').forEach(b=>b.onclick=()=>{
        r.observations=r.observations.filter(o=>String(o.id)!==String(b.dataset.delObs));
        saveState();
        renderReussir();
      });
      ...
      semesterObs.forEach(o=>{
        const student=o.student==='Adefarayola'?'Fara':
          (o.student==='Araotanlowooluwa'||o.student==='Araotanlowooluwa')?'Araotan':o.student;
        byStudent[student]=byStudent[student]||{};
        const k=o.label||o.key||'';
        byStudent[student][k]=(byStudent[student][k]||0)+1;
      });

      panel.innerHTML=`
        <section class="hh-list-card">
          <div class="hh-section-title"><div><strong>Sommaire</strong><span>Observations par élève</span></div></div>
